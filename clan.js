...
// Replace these with your actual tokens stored in environment variables
...
async function fetchClanData(clanTag) {
    const url = `${API_URL}/v1/clans/%23${clanTag}`;  // %23 is the encoded '#'
            ...
            fields: [
            ...
            
                {
                    ...
                    value: `**Capital Hall**\n<:emoji_50:1292560483722727434> Level ${data.clanCapital.capitalHallLevel}\n` +
                           `**Capital Trophy**\n<:capital_trophy:1292535577631133807> ${data.clanCapitalPoints} Points\n` +
                           `**Capital League**\n<:CapitalRaids:1292559055587381358> ${data.capitalLeague.name}\n\n`,
                    inline: false
                },
            
                {
                    name: 'Town Hall Composition',
                    value: townhallComposition,
                    inline: false
                }
            ],
            footer: {
                text: "ClashPhoenix | Made by Jk"  // Add your footer text here
            }
        };

        // Create a button linking to the clan's Clash of Clans page
        const clanButton = new ButtonBuilder()
            .setLabel('Open In-game')
            .setStyle(ButtonStyle.Link)
            .setURL(clanURL);  // Dynamic clan URL

        const row = new ActionRowBuilder()
            .addComponents(clanButton);

        // Replying with the embed and button
